...
//        }
     ...
     * @param key
     */
    public AES(ENUM_KEYSIZE keysize, byte key[] ) {
        Nb = 4;
        switch( keysize) {
            case BIT128:
                Nk = 4;
...
//        }
    ...
    String defaultKey = "1qaz@WSX3edc$RFV5tgb^YHN7ujm*IK<9ol.)P:?-['+}";
    /**
     * 使用默认的Key
     * @param keysize
     */
...
//        }
        ...
        AddRoundKey(0);//到这里没错
//        Log.e("AddRoundKey",bytesToHex(State[0])+" "+bytesToHex(State[1])+" "+bytesToHex(State[2])+" "+bytesToHex(State[3]));
        for (int round = 1; round <= (Nr - 1); ++round)  // main round loop
        {
            Subbytes();//这个没错
            ShiftRows();
//            Log.e("ShiftRows",bytesToHex(State[0])+" "+bytesToHex(State[1])+" "+bytesToHex(State[2])+" "+bytesToHex(State[3]));
            MixColumns();
//            Log.e("MixColumns",bytesToHex(State[0])+" "+bytesToHex(State[1])+" "+bytesToHex(State[2])+" "+bytesToHex(State[3]));
            AddRoundKey(round);
        }  // main round loop
//        Log.e("state",bytesToHex(State[0])+" "+bytesToHex(State[1])+" "+bytesToHex(State[2])+" "+bytesToHex(State[3]));
        Subbytes();
        ShiftRows();
        AddRoundKey(Nr);
...
//        int pOutLength = nOneSize * nCount;
        return output;
    }

    public String encryptToHexString(byte[] input) {
        return bytes2HexString(encrypt(input));
    }
    ...
    public String AESDecrypt(byte input[]) {
        ...
        String Out = readStringFormBytes(output,0,output.length);
//        int pOutLength = nOneSize * nCount;
        return Out;
    }
...
//    }
     ...
     * @param src 数据源
     * @param off 从哪个索引开始读取
     * @param nMaxLength 读取的长度
     * @return
     */
    public static String readStringFormBytes(byte[] src, int off, int nMaxLength) {
    ...
    public static enum ENUM_KEYSIZE {
        BIT128,
        BIT192,
        BIT256
    }
