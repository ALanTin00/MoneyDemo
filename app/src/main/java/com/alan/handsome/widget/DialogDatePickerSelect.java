    ...
    public void showDatePickView(String title, int flag) {
        ...
        view.findViewById(R.id.dds_confirm_tv).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                long time = Long.parseLong(simpleFormatter.format(calendar.getTime()).replaceAll("/",""));
                callBack.onDateSelected(flag, datePicker.getYear(), datePicker.getMonth() + 1, datePicker.getDayOfMonth(), time, simpleFormatter.format(calendar.getTime()));
                dialog.dismiss();
            }
        });
        ...
        for (int i = 0; i < mSpinners.getChildCount(); i++) {
            ...
            for (Field pf : pickerFields) {
                if (pf.getName().equals("mSelectionDivider")) {
                    ...
                    try {
                        pf.set(picker, new ColorDrawable(ContextCompat.getColor(context, R.color.color_33)));
